...
public class DtoMakeTest extends TestCase {
	...
	public void testMakeRsp() throws Exception {
		InputStream resourceAsStream = DtoMakeTest.class.getClassLoader()
				.getResourceAsStream("ThostFtdcUserApiDataType.h");
		InputStream resourceAsStream2 = DtoMakeTest.class.getClassLoader().getResourceAsStream("dto.rsp");

		List<String> readLines = IOUtils.readLines(resourceAsStream, Charsets.UTF_8);
		...
		List<String> readLines2 = IOUtils.readLines(resourceAsStream2, Charsets.UTF_8);
		for (String line : readLines2) {
			...
			if (split.length == 3) {
				...
				if (value == 4) {
					...
					methodParseFrom.append("info.set" + StringUtils.capitalize(prop) + "(body.readInt());")
							.append(LINE);
				} else if (value == 8) {
					propertiesAll.append("private double ").append(prop).append(";").append(LINE);
					methodParseFrom.append("info.set" + StringUtils.capitalize(prop) + "(body.readDouble());")
							.append(LINE);
				} else {
					...
					methodParseFrom.append("info.set" + StringUtils.capitalize(prop)
							+ "(StringUtils.trimToEmpty(new String(" + prop + ")));").append(LINE);
				}
		...
		classAll.append(propertiesAll.toString()).append(LINE).append(methodParseFrom.toString()).append(LINE)
				.append("}");
	...
	public void testMakeReq() throws Exception {
		...
		for (String line : readLines2) {
			...
			if (split.length == 3) {
				...
				if (value == 4) {
					...
					methodProp.append("public void set" + StringUtils.capitalize(prop) + "(int " + prop + ") {")
							.append(LINE);
					...
					methodProp.append("public void set" + StringUtils.capitalize(prop) + "(double " + prop + ") {")
							.append(LINE);
					methodProp.append("this." + prop + " = " + prop + ";").append(LINE);
					methodProp.append("}").append(LINE);
				} else {
					...
					methodProp.append("public void set" + StringUtils.capitalize(prop) + "(String " + prop + ") {")
							.append(LINE);
					methodProp.append("if(StringUtils.isNotEmpty(" + prop + "))").append(LINE).append("\t");
					methodProp.append("System.arraycopy(" + prop + ".getBytes(), 0, this." + prop + ", 0, " + prop
							+ ".getBytes().length);").append(LINE);
					methodProp.append("}").append(LINE);
				}
		...
		classAll.append(propertiesAll.toString()).append(LINE).append(methodProp.toString()).append(LINE)
