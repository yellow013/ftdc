...
import io.ffreedom.ftd.dto.FtdcReq;
import io.ffreedom.ftd.dto.ReqAuth;
...
import io.ffreedom.ftd.handler.task.RecieveMessageTimerTask;
...
public class FtdcTraderApiAdapter implements FtdcTraderApi {
	...
	public void reqUserLogin(RequestIdentity requestIdentity, FtdcReq userLogin) {
		...
		pool.acquire(sas)
				.addListener(new LoginListener(requestIdentity, TID.UserLoginReq, Sequence.UserLogin, userLogin));
	}
	...
	public void reqUserLogout(RequestIdentity requestIdentity, FtdcReq userLogout) {
		...
		Channel ftdcChannel = ApplicationRuntime.getFtdcChannel(requestIdentity.getBrokerId(),
	...
	public void reqOrderInput(RequestIdentity requestIdentity, FtdcReq inputOrder) {
		...
		if (ftdcChannel.isActive()) {
			...
			FtdcProtocol ftdc = new FtdcProtocol(FtdType.FTDTypeCompressed, buffer, FtdcType.REQ.type(),
					requestIdentity.getReqId(), TID.OrderInsertReq.id(), Sequence.OrderInsert);
			ftdcChannel.writeAndFlush(ftdc);
	...
	abstract class AbstractFtdcFutureListener implements GenericFutureListener<Future<Channel>> {
		...
		public void operationComplete(Future<Channel> future) throws Exception {
			...
			if (future.isSuccess()) {
				...
				channel.writeAndFlush(new FtdcProtocol(FtdType.FTDTypeCompressed, buffer, FtdcType.REQ.type(),
						requestIdentity.getReqId(), tid.id(), sequence));
	...
	class AuthListener extends AbstractFtdcFutureListener {
		...
		public AuthListener(RequestIdentity requestIdentity, TID tid, Sequence sequence, FtdcReq reqUserLogin) {
	...
	class LoginListener extends AbstractFtdcFutureListener {
		...
		public LoginListener(RequestIdentity requestIdentity, TID tid, Sequence sequence, FtdcReq reqUserLogin) {
			super(requestIdentity, tid, sequence);
