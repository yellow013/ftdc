...
import io.ffreedom.ftd.ftdc.FtdHeartbeatHandler;
import io.ffreedom.ftd.ftdc.FtdcDecoder;
import io.ffreedom.ftd.ftdc.FtdcEncoder;
import io.ffreedom.ftd.ftdc.FtdcHandler;
import io.ffreedom.ftd.pool.SocketAddressChooserFactory.SocketAddressChooser;
...
public class FtdClientPool {
	...
	private void initPoolMap() {
		this.pollMap = new NbtsChannelPoolMap(ApplicationRuntime.conf().getMaxConnections(),
				ApplicationRuntime.conf().getMaxPending());
	}
	...
	static class NbtsChannelPoolMap extends AbstractChannelPoolMap<InetSocketAddress, FixedChannelPool> {
		...
		protected FixedChannelPool newPool(InetSocketAddress key) {
			return new FixedChannelPool(initBootStrap().remoteAddress(key), new InitChannelPoolHandler(),
					new DefaultChannelHealthChecker(), AcquireTimeoutAction.NEW, 500, maxConnections, maxPending);
		}
	...
	static class DefaultChannelHealthChecker implements ChannelHealthChecker {
		...
		public Future<Boolean> isHealthy(Channel channel) {
			...
			if (!channel.isActive()) {
				logger.warn("local addr: {}, remote addr {} , channel not active, closed..", channel.localAddress(),
	...
	static class InitChannelPoolHandler implements ChannelPoolHandler {
		...
		public void channelCreated(Channel ch) {
			...
			ch.pipeline()
					.addLast(new IdleStateHandler(ApplicationRuntime.conf().getFtdcReaderIdle(),
							ApplicationRuntime.conf().getFtdcWriterIdle(), ApplicationRuntime.conf().getFtdcAllIdle(),
							TimeUnit.SECONDS));
